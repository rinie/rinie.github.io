...
One hundred and five posts, one coherent framework applied at every layer from printing presses to parking cars.
...
| 102 | [They Bought a Porsche and Had No Money Left for Gas](https://rinie.github.io/2026/08/24/porsche-no-gas-money/) | Aug 24 | Friedrichshafen's T-City spent millions on visible smart-city showpieces starting in 2007 and left the town hall's paperwork untouched. A rural German district on the Dutch border did the opposite in 2013 — digitised the boring internal workflows first, built a shared anti-lock-in interchange standard with competing vendors, and only then built anything a citizen would see. |
| 103 | [The Diamond Shows Up Eventually: Network, Relational, and Object Databases](https://rinie.github.io/2026/08/25/the-diamond-shows-up-eventually/) | Aug 25 | A network database is a building where the only way from Accounting to Payroll is a specific elevator shaft installed when the building went up. Codd's relational model isolated the Gutenberg/Semantic boundary in exactly one place instead of smearing it everywhere. Object databases repeated the network mistake decades later, with better marketing — the hourglass out-evolved the diamond, the same reason S3 out-evolved HDFS. |
| 104 | [The Diamond Moves, It Doesn't Disappear: ORM's Hidden Impedance](https://rinie.github.io/2026/08/26/the-diamond-moves-it-doesnt-disappear/) | Aug 26 | ORMs market themselves as solving impedance mismatch. order.customer.address looks like a field access but it's SELECT-chasing underneath — the coupling didn't disappear, it moved somewhere invisible. XML and C++ do the same fusion at the schema and memory-layout level. C's ABI, not C++'s, became the universal interchange layer, for the exact reason S3's did. |
